...
function TaskListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TaskInterface[]>(
    ...
    () =>
      getTasks({
        relations: ['user_task_assigned_toTouser', 'user_task_created_byTouser', 'organization'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_task_assigned_toTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_task_created_byTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_task_assigned_toTouser?.id}`}>
                          {record.user_task_assigned_toTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_task_created_byTouser?.id}`}>
                          {record.user_task_created_byTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
